...
local SynCityFolder = "SynCity"
local ConfigurationFolder = SynCityFolder.."/Configurations"
...
local SynLibrary = {
	...
	Theme = {
		Default = {
			TextFont = "Gotham", -- Default will use the various font faces used across Syn
			TextColor = Color3.fromRGB(120, 81, 169),

			Background = Color3.fromRGB(25, 25, 25),
			Topbar = Color3.fromRGB(0, 0, 0),
			Shadow = Color3.fromRGB(5, 2, 3),

			NotificationBackground = Color3.fromRGB(20, 20, 20),
			NotificationActionsBackground = Color3.fromRGB(230, 230, 230),

			TabBackground = Color3.fromRGB(6, 2, 3),
			TabStroke = Color3.fromRGB(121, 81, 169),
			TabBackgroundSelected = Color3.fromRGB(83, 69, 130),
			TabTextColor = Color3.fromRGB(120, 81, 169),
			...
			SliderBackground = Color3.fromRGB(120, 81, 169),
			SliderProgress = Color3.fromRGB(120, 81, 169),
			SliderStroke = Color3.fromRGB(6, 2, 3),
		...
		Light = {
			...
			TabBackgroundSelected = Color3.fromRGB(83, 69, 130),
			TabTextColor = Color3.fromRGB(120, 81, 169),
			SelectedTabTextColor = Color3.fromRGB(152 ,29, 151),
...
local Syn = game:GetObjects("rbxassetid://10804731440")[1]
...
elseif CoreGui:FindFirstChild("RobloxGui") then
	Syn.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Syn.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Syn.Name and Interface ~= Syn then
			Interface.Enabled = false
			Interface.Name = "Syn-Old"
		end
	end
else
...
local Notifications = Syn.Notifications

local SelectedTheme = SynLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = SynLibrary.Theme[ThemeName]
	for _, obj in ipairs(Syn:GetDescendants()) do
	...
	Syn.Main.BackgroundColor3 = SelectedTheme.Background
	Syn.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Syn.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Syn.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Syn.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Syn.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Syn.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if SynLibrary.Flags[FlagName] then
			spawn(function() 
				if SynLibrary.Flags[FlagName].Type == "ColorPicker" then
					SynLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if SynLibrary.Flags[FlagName].CurrentValue or SynLibrary.Flags[FlagName].CurrentKeybind or SynLibrary.Flags[FlagName].CurrentOption or SynLibrary.Flags[FlagName].Color ~= FlagValue then SynLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			SynLibrary:Notify({Title = "Flag Error", Content = "Syn was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(SynLibrary.Flags) do
...
function SynLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= SynLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Syn | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	SynLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function SynLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Syn Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Syn Interface Suite" then
    ...
    ImageFrame.Size = UDim2.new(0, 0, 0, 0.1)
    ImageFrame.Position = UDim2.new(0, 0, 0, 100)
    ...
    ImageFrame.Size = UDim2.new(0, 0, 0.1, 0.1)
	...
	pcall(function()
		...
		if not isfolder(SynCityFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(SynCityFolder.."/Discord Invites") then
			makefolder(SynCityFolder.."/Discord Invites")
		end
		if not isfile(SynCityFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(SynCityFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Syn RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(SynCityFolder.."/Key System") then
			makefolder(SynCityFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Syn | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(SynCityFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(SynCityFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
		...
		if not Passthrough then
			...
			Syn.Enabled = false
			local KeyUI = game:GetObjects("rbxassetid://13320180666")[1]
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(SynCityFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						SynLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Syn | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					SynLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Syn | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				if DropdownSettings.CurrentOption == Option then
					DropdownOption.BackgroundColor3 = Color3.fromRGB(240, 240, 240)
				end
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if table.find(DropdownSettings.CurrentOption, Option) then
						...
						TweenService:Create(DropdownOption, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(240, 240, 240)}):Play()
						Debounce = true
						wait(0.2)
					...
					if not Success then
						...
						print("Syn | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			for _, droption in ipairs(Dropdown.List:GetChildren()) do
				if droption.ClassName == "Frame" and droption.Name ~= "Placeholder" then
					if not table.find(DropdownSettings.CurrentOption, droption.Name) then
						...
						droption.BackgroundColor3 = Color3.fromRGB(240, 240, 240)
					end
				end
			end
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					SynLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Syn | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					SynLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Syn | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					SynLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Syn | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					SynLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function SynLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				SynLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, SynLibrary.LoadConfiguration, SynLibrary)
